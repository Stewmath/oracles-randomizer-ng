...
func findRoute(rom *romState, seed uint32, src *rand.Rand,
	...
	// Shuffle music (questionable whether this should be part of a "route", but everything
	// using the "src" rng seed is called from this function)
	shuffleMusic(rom, ropts, ri)

	return ri, nil
}

func shuffleMusic(rom *romState, ropts randomizerOptions, ri *routeInfo) {
	...
	musicList := rom.musicList[:]

	if ropts.musicShuffle == "all" {
		musicList = append(musicList, rom.customMusicList...)
	}

	ri.src.Shuffle(len(musicList), func(i, j int) {
